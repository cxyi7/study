...
17. 描述事件冒泡的流程
	  1. 在嵌套的父子元素中，子元素的事件会一层一层向上传递，一直到顶层元素window对象
		2. 补充：event.stopPropagation();  可以阻止捕获和冒泡

18. 什么是事件代理
	  1. 事件代理就是在父元素上绑定事件，然后通过父元素的事件来处理子元素的相关操作。
		2. 好处是代码简洁，减少浏览器内存
...
30. 描述一下HTTP的缓存机制
    1. 为什么要使用缓存？ 可以减少网络请求的数量和体积，提高页面显示速度，使用户体验更好
		...
		3. 什么是强制缓存？设置Cache-control(Response Headers中)：max-age(设置过期时间)，no-cache(强制向服务器发送请求，由服务器判断该资源是否有更新，有则返回新内容，没有则使用缓存)，no-store(不用本地缓存，也不使用服务端的缓存措施)，private(针对个人用户)，public(允许中间代理或路由进行缓存处理)
		4. 什么是协商缓存？
		    1. 由服务器判断客户端资源是否和服务端资源一样，一致则返回304，否则返回200和最新的资源
	      2. 第一次请求时，会返回资源和资源标识，通过判断这个资源标识来辨别资源是否有更新
